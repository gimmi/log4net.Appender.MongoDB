...
Task Compile -Depends Deps, AssemblyInfo {
	...
	Exec { msbuild "$build_dir\src\log4net.Appender.MongoDB.sln" /t:Build /p:Configuration=$build_cfg /v:quiet /nologo }
}
...
Task Publish -Depends Clean, Test {
    ...
    New-Item $build_dir\build -type directory
   	...
   	Exec { & "$build_dir\tools\nuget\nuget.exe" pack "$build_dir\src\log4net.Appender.MongoDB\log4net.Appender.MongoDB.csproj" -Build -OutputDirectory $build_dir\build, -Symbols -Prop Configuration=$build_cfg }
	Exec { & "$build_dir\tools\nuget\nuget.exe" push "$build_dir\build\log4net.Appender.MongoDB.$version.nupkg" }
